...
function initializeServiceWorker() {
  ...
  window.addEventListener('load', () => {
    navigator.serviceWorker
      .register('sw.js')
      .then(reg => console.log('Successfully loaded service worker'))
    });
  }
...
function createRecipeCards() {
  for (let i = 0; i < Object.keys(recipeData).length; i++){
    ...
    const single = recipeData[recipes[1]]
    console.log(single);
...
function bindEscKey() {
  ...
  document.addEventListener('keydown', function(event){
    if(event.key === "Escape"){
      ...
      console.log("pressed on escape key");
      router.navigate("home", false);
      history.pushState(null, null, "stuffing");
    }
  });
...
function bindPopstate() {
  ...
  document.addEventListener('keydown', function(event) {
      ...
      if (event.key == "ArrowLeft"){
        ...
        // window.history.back();
        router.navigate('backward', true)
      }else if (event.key == "ArrowRight"){
        console.log("going forward");
        // window.history.forward();
        router.navigate('forward', true)
      }
